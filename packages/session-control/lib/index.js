...
const Config = Schema.object({
	allowGlobalAccess: Schema.boolean().default(true),
	allowArchive: Schema.boolean().default(false),
	maxManagedSessions: Schema.number().step(1).min(1).default(12)
});
const TEXT_OUTPUT = {
	schema: { type: "string" },
	render: (_args, value) => [{
...
const SESSION_VISIBILITY = "Global access is deployment-configurable. Created sessions remain visible in the normal Web sidebar.";
const SETTINGS_NAMESPACE = "session-control";
const SETTINGS_ROUTE = "/api/session-control/settings";
const RuntimeSettings = Schema.object({ allowGlobalAccess: Schema.boolean().default(true) });
...
function defineSessionTool(options) {
	...
	return {
		...
		parameters: {
			type: "object",
			properties: Object.fromEntries(Object.entries(options.parameters).map(([key, value]) => [key, {
				type: value.type,
		...
		...options.isConcurrencySafe === void 0 ? {} : { isConcurrencySafe: (args) => options.isConcurrencySafe(args) },
...
function apply(ctx, config = {}) {
	...
	ctx.effect(() => () => Promise.all([...handles.values()].map((handle) => handle.dispose())), "session_control.dispose");
	ctx.tools.register(defineSessionTool({
		...
		async execute(args, exec) {
			...
			const workspace = await resolveCreationWorkspace(ctx, parent, args.workspace_id, globalAccessEnabled());
			if (handles.size >= resolved.maxManagedSessions) throw new Error(`session_control: managed-session limit (${resolved.maxManagedSessions}) reached`);
	...
	ctx.tools.register(defineSessionTool({
		...
		async execute(_args, exec) {
			...
			const statuses = await Promise.all(workspaces.map((workspace) => workspace.status()));
			return workspaces.map((workspace, index) => `${String(workspace.id)} [${statuses[index]}] [${workspace.sessionIds.length} sessions] - ${workspace.title} - ${workspace.path}`).join("\n");
		}
	}));
	ctx.tools.register(defineSessionTool({
		...
		async execute(args) {
			...
			const workspace = await ctx.workspaceRegistry.create(nonBlank("path", args.path), args.title === void 0 ? void 0 : nonBlank("title", args.title));
	...
	ctx.tools.register(defineSessionTool({
		...
		output: TEXT_OUTPUT,
		async execute(args, exec) {
			const workspace = await requireManagedWorkspace(ctx, requireAgent(exec), workspaceId(args.workspace_id), globalAccessEnabled());
	...
	ctx.tools.register(defineSessionTool({
		...
		async execute(args, exec) {
			...
			await requireManagedWorkspace(ctx, requireAgent(exec), target, globalAccessEnabled());
			return await ctx.workspaceRegistry.delete(target) ? `Removed workspace registration ${String(target)}. Directory and session logs were retained.` : `Workspace ${String(target)} was already absent.`;
		}
	}));
	ctx.tools.register(defineSessionTool({
		...
		async execute(args, exec) {
			...
			return formatSessionEntries(ctx, workspace.sessionIds.map((id) => ({
				id,
	...
	ctx.tools.register(defineSessionTool({
		...
		async execute(args, exec) {
			...
			if (target === parent.id) throw new Error("session_control: a session cannot stop itself");
			const agent = ctx.agents.get(target);
			if (agent === void 0 || agent.status !== "running") return `Session ${String(target)} is not running.`;
			agent.cancel({ kind: "user" }, { keepInbox: true });
			return `Stop requested for session ${String(target)}.`;
		}
	}));
	if (resolved.allowArchive) ctx.tools.register(defineSessionTool({
		...
		async execute(args, exec) {
			const parent = requireAgent(exec);
			const target = await requireManagedSession(ctx, parent, sessionId(args.session_id), globalAccessEnabled());
			if (target === parent.id) throw new Error("session_control: a session cannot archive itself");
			if (ctx.agents.get(target)?.status === "running") throw new Error("session_control: stop the target before archiving it");
...
function resolveConfig(config) {
	...
	if (!Number.isSafeInteger(maxManagedSessions) || maxManagedSessions < 1) throw new TypeError("session_control: maxManagedSessions must be a positive safe integer");
	return {
...
async function formatSessionEntries(ctx, entries) {
	const titles = await ctx.sessionQuery.readTitleSnapshots(entries.map((entry) => entry.id));
...
async function globalSessionEntries(ctx) {
	...
	for (const workspace of ctx.workspaceRegistry.list()) for (const id of workspace.sessionIds) locations.set(id, `workspace:${workspace.title}`);
...
function requireGlobalAccess(allowGlobalAccess) {
	if (!allowGlobalAccess) throw new Error("session_control: this operation requires global access; enable it in Settings > Plugins");
}
async function requireManagedWorkspace(ctx, agent, target, allowGlobalAccess) {
	...
	if (workspace === void 0) throw new Error(`session_control: unknown workspace ${String(target)}`);
	if (allowGlobalAccess) return workspace;
	const callerWorkspace = await requireCallerWorkspace(ctx, agent);
	if (workspace.id !== callerWorkspace.id) throw new Error(`session_control: workspace ${String(target)} is outside the caller workspace`);
	return workspace;
}
async function resolveCreationWorkspace(ctx, agent, requestedId, allowGlobalAccess) {
	...
	return requireManagedWorkspace(ctx, agent, workspaceId(nonBlank("workspace_id", requestedId)), allowGlobalAccess);
}
...
async function handleSettingsRequest(req, res, settings) {
	...
	try {
		...
		await settings.update({ allowGlobalAccess: body.allowGlobalAccess });
		sendJson(res, 200, settings.get());
	} catch (error) {
...
function requireAgent(exec) {
	if (exec.agent === void 0) throw new Error("session_control: these tools require an agent-bound caller");
	return exec.agent;
}
...
async function requireCallerWorkspace(ctx, agent) {
	...
	if (cwd === void 0) throw new Error("session_control: caller has no workspace directory");
	const workspace = await ctx.workspaceRegistry.resolveByPath(cwd);
	if (workspace === void 0) throw new Error(`session_control: no registered workspace owns ${cwd}`);
	return workspace;
}
async function requireWorkspaceSession(ctx, agent, target) {
	if (!(await requireCallerWorkspace(ctx, agent)).sessionIds.includes(target)) throw new Error(`session_control: session ${String(target)} is outside the caller workspace`);
	return target;
}
async function requireManagedSession(ctx, agent, target, allowGlobalAccess) {
	...
	if (!(await ctx.sessionPersistence.list()).some((header) => header.id === target)) throw new Error(`session_control: unknown session ${String(target)}`);
	return target;
}
async function ensureResident(ctx, handles, sessionId, caller) {
