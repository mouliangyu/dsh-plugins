...
import type { ConnectionHandle, HostFrame, IApiClient, MuxFrame, RpcRequest, SessionId } from '@deepseek-ai/dsh-api-remotes/client'
...
export type { AuthorityConnectionLike, AuthorityProviderLike, AuthorityRegistryLike, AuthorityState } from './authority-types.ts'
...
export type { SubagentAddress, JobView } from '@deepseek-ai/dsh-api-remotes/client'
...
interface RoutedConnectionHandle extends ConnectionHandle {
...
export function apply(ctx: Context): void {
  ...
  const connection = ctx.get('connection') as RoutedConnectionHandle
  const authorities = ctx.get('authorityRegistry') as AuthorityRegistryLike
...
class RemoteAuthorityStreams {
  ...
  constructor(private readonly registry: AuthorityRegistryLike, private readonly router: AuthorityApiRouter, private readonly sinks: RemoteStreamSinks) {}
  ...
  private async pump(id: string, api: IApiClient, signal: AbortSignal): Promise<void> {
    ...
    let announced = false
    const opened = (): void => {
      if (!announced && muxOpen && hostOpen) { announced = true; this.sinks.onConnected() }
    }
  ...
  private async consume<F>(stream: AsyncIterable<RpcRequest<F>>, sink: (envelope: RpcRequest<F>) => void): Promise<void> {
    try { for await (const envelope of stream) sink(envelope) }
    catch (error) { console.warn('[dsh-remote] authority stream ended:', error) }
  }
}
