...
import type { SessionRelayService } from "dsh-session-control/relay";
import {
  SESSION_RELAY_PATH,
  SESSION_RELAY_PROTOCOL_VERSION,
  parseSessionRelayFrame,
...
export const name = "dsh_remote_relay_channel";
/** Services required to expose the relay transport. */
export const inject = ["sessionRelay", "webServer"];
...
export const Config: z<Config> = z.object({
  requestTimeoutSeconds: z.number().step(1).min(1).max(300).default(30),
});

/** Expose the session relay service through the dsh-remote WebSocket protocol. */
export function apply(ctx: Context, config: Config = {}): void {
  const requestTimeoutMs = (config.requestTimeoutSeconds ?? 30) * 1000;
  const relay = ctx.get("sessionRelay") as SessionRelayService;
  ctx.effect(
    () => registerSessionRelayChannel(ctx, relay, requestTimeoutMs),
    "dsh-remote.relayChannel",
  );
}

function registerSessionRelayChannel(
  ctx: Context,
  relay: SessionRelayService,
  ...
  const disposeUpgrade = ctx.webServer.registerUpgrade({
    ...
    handler: (_req: IncomingMessage, socket: Duplex, head: Buffer) => {
      server.handleUpgrade(_req, socket, head, connected => {
        ...
        connected.once("message", (data, isBinary) => {
          ...
          try {
            ...
            if (hello.type !== "hello" || hello.version !== SESSION_RELAY_PROTOCOL_VERSION) {
              throw new Error("first session relay frame must be a compatible hello");
            }
            ...
            let disposeProvider = () => {};
            const provider = new SessionRelaySocketProvider(
              hello.peerId,
              connected,
              {
                receive: message => relay.receive({
                  ...message,
                  from: { authorityId: hello.peerId, sessionId: message.from.sessionId },
            ...
            connected.send(JSON.stringify({ type: "ready", version: SESSION_RELAY_PROTOCOL_VERSION }));
          } catch (error) {
