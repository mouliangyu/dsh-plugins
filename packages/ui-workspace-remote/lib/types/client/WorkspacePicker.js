 ...
 */
export function WorkspacePickFlow({ t, open, anchorRef, useWorkspaces, createWorkspace, authorityRegistry, selectDirectoryAuthority, useDirectoryFlow, renderDirectoryFlow, onPick, onClose, addOnly = false, side = 'bottom', selectedId, }) {
    const workspaceSnapshot = useWorkspaces(state => state);
    const authoritySnapshot = useSyncExternalStore(listener => authorityRegistry.subscribe(listener), () => authorityRegistry.getSnapshot());
    ...
    const addEntries = flowAvailable
        ? [
            { id: ADD_LOCAL_WORKSPACE, label: t('menu.addLocalWorkspace'), icon: _jsx(IconPlusOutline16, { size: 16 }), disabled: flowBusy },
            ...readyAuthorities.map(id => ({ id: `${ADD_REMOTE_PREFIX}${id}`, label: t('menu.addRemoteWorkspace').replace('{remote}', id), icon: _jsx(IconPlusOutline16, { size: 16 }), disabled: flowBusy })),
        ]
        : [];
    ...
    const openDirectoryFlow = useCallback((authorityId) => {
 ...
 */
export function WorkspacePicker({ open, anchorRef, useWorkspaces, selectedId, onPick, onClose, createWorkspace, authorityRegistry, selectDirectoryAuthority, useDirectoryFlow, renderSlot, t, }) {
    return (_jsx(WorkspacePickFlow, { t: t, open: open, anchorRef: anchorRef, useWorkspaces: useWorkspaces, createWorkspace: createWorkspace, authorityRegistry: authorityRegistry, selectDirectoryAuthority: selectDirectoryAuthority, useDirectoryFlow: useDirectoryFlow, renderDirectoryFlow: owner => renderSlot('conversation.hero.workspace.directoryFlow', owner), selectedId: selectedId, onPick: onPick, onClose: onClose }));
}
