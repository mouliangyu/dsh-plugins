...
export function WorkspacePickFlow({
  ...
  const addEntries: MenuEntry[] = flowAvailable
    ? [
      { id: ADD_LOCAL_WORKSPACE, label: t('menu.addLocalWorkspace'), icon: <IconPlusOutline16 size={16} />, disabled: flowBusy },
      ...readyAuthorities.map(id => ({ id: `${ADD_REMOTE_PREFIX}${id}`, label: t('menu.addRemoteWorkspace').replace('{remote}', id), icon: <IconPlusOutline16 size={16} />, disabled: flowBusy })),
    ]
    : []
